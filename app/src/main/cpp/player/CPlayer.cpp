...
void CPlayer::prepare_() {
    ...
    for (int i = 0; i < formatContext->nb_streams; ++i) {
        ...
        if(r<0){
            onError(THREAD_CHILD,r,"解码器参数拷贝");
            LOGD("error：解码器参数拷贝");
            return;
        }
        //打开编码器
    ...
    if(!video_channel&&!audio_channel){
        ...
        onError(THREAD_CHILD,-1,"音视频channel异常");
        return;
    }
    //回调prepared 成功
    if(helper){
...
//开始播放视频
void CPlayer::start_() {
    video_channel->player_(formatContext);
}
